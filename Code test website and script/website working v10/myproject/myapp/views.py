...
def run_script(request):
    if request.method == 'POST':
        ...
        Failed, ShovedIn = panelize_pcb(updated_uploaded_files, 0, output_file, (pcbnew.FromMM(maxWidth), pcbnew.FromMM(maxHeight)))
...
def panelize_pcb(uploaded_files, spacing, output_file, boxSize):
    ...
    for i in range(len(sortedboards)):
        j = order[i]
        phaseDisplacement = displacement[j]
        if not phaseDisplacement: continue

        board = sortedboards[j]
        phaseDisplacement -= absolute_displacement
        absolute_displacement += phaseDisplacement
 
        panel_board = merge_boards(panel_board, board, phaseDisplacement, f"-v{j}-")
