...
def user_files(request):
    ...
    return render(request, 'user_files.html', {'resultfiles': resultfiles, 'selected_files':selected_files, 'board_height':board_height, 'board_width':board_width})
...
def run_script(request):
    if request.method == 'POST':
        ...
        Failed, ShovedIn = panelize_pcb(uploaded_files, 10, output_file, (pcbnew.FromMM(maxWidth), pcbnew.FromMM(maxHeight)))
...
def panelize_pcb(uploaded_files, spacing, output_file, boxSize):
    ...
    originPos = panel_board.GetBoardEdgesBoundingBox().GetPosition()
    ...
    displacement, Failed, ShovedIn = fit_rectangles(rectangles, boxSize, spacing)
...
def fit_rectangles(rectangles, boxSize, spacing):
    ...
    def putItIn(rectangles, box, rect):
        ...
        delta_x = 0
        delta_y = 0
        delta_x_max = pcbnew.ToMM(box.GetX() + box.GetWidth() - rect.GetWidth())
        delta_y_max = pcbnew.ToMM(box.GetY() + box.GetHeight() - rect.GetHeight())

        if len(rectangles) == 0 and isInsideBox(rect, box): return (0, 0)
        ...
        while not ImGood and (delta_x <= delta_x_max or delta_y < delta_y_max):
            ...
            tmp_rect = pcbnew.BOX2I(rect.GetPosition() + pcbnew.VECTOR2I(pcbnew.FromMM(delta_x), pcbnew.FromMM(delta_y)), rect.GetSize())
        ...
        return (delta_x, delta_y) if delta_x <= delta_x_max and delta_y <= delta_y_max else None
    ...
    def goForIt(rectanglesIn, box):
        ...
        while len(rectangles) != 0:
            ...
            rect = rectangles.pop()
            vector = putItIn(shovedIn, box, rect)
            if not vector:
                Failed.append(rect)
                translation_vectors.append(None) #append None
            else:
                delta_x, delta_y = vector
                translation_vectors.append(pcbnew.VECTOR2I(pcbnew.FromMM(delta_x), pcbnew.FromMM(delta_y)))
                shovedIn.append(pcbnew.BOX2I(rect.GetPosition()+ pcbnew.VECTOR2I(pcbnew.FromMM(delta_x), pcbnew.FromMM(delta_y)), rect.GetSize()))
    ...
    return translation_vectors, Failed, shovedIn
