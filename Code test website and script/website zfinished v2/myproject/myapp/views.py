...
def logged_home(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return deposit_project(request)
...
def send_panel_prod(request, panel_id):
    ...
    if panel_file.valid or request.user.is_staff:
        panel_file.isToProduce = True
        panel_file.update_status()
        panel_file.save()
    
    return redirect('user-panels')
...
def step_panel(request, panel_id):
    ...
    if panel_file.isToProduce:
        ...
        panel_file.productionStep = ProductionStep.choices[0]
    ...
    if panel_file.productionStep == ProductionStep.choices[-1]:
